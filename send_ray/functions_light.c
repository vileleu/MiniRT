...
/*   Updated: 2020/07/21 18:06:56 by vileleu          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	final_light(t_lum *light, t_close *inter, double i)
{
	...
	if (light->color.z && !inter->col_form.z)
		inter->col_form.z = 1;
	inter->col_final.x += light->ratio * i * 10 * \
	light->color.x * inter->col_form.x;
	inter->col_final.y += light->ratio * i * 10 * \
	light->color.y * inter->col_form.y;
	inter->col_final.z += light->ratio * i * 10 * \
	light->color.z * inter->col_form.z;
