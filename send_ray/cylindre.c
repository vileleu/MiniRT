...
/*   Updated: 2020/09/05 16:02:19 by vileleu          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	solutioncyb(t_close *inter, t_cy *cy, t_ray ray, double smallest)
{
	t_vect	caps;
	...
	if ((inter->solu == 0 && smallest > 0) || smallest < inter->solu)
	{
		...
		caps = ope('+', cy->crdn, multi(cy->orientation, cy->height / 2));
		inter->normale = normalize(ope('-', inter->inter, caps));
...
void	solutioncy(t_close *inter, t_cy *cy, t_ray ray, double smallest)
{
	...
	if ((inter->solu == 0 && smallest > 0) || smallest < inter->solu)
	{
		...
		caps[1] = normalize(ope('-', inter->inter, caps[0]));
...
void	inter_cybis(t_scene s, t_close *inter, t_ray ray)
{
	...
	if (cal[5] > 0 && cal[5] < cal[6] && cal[4] > 0)
		solutioncy(inter, s.cy, ray, cal[4]);
	cal[4] = (((cal[5] < 0) ? 0 : cal[6]) - cal[8]) / cal[7];
	if (absolue(cal[1] + cal[0] * cal[4]) < cal[3])
		solutioncyb(inter, s.cy, ray, cal[4]);
}
